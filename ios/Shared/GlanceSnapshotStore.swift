...
enum GlanceSnapshotStore {
    ...
    static func save(_ snap: Snapshot) {
        ...
        if let kwh = snap.todayKwh { defaults?.set(kwh, forKey: Key.todayKwh) }
        if let cost = snap.todayCostFiat { defaults?.set(cost, forKey: Key.todayCost) }
    ...
    static func load() -> Snapshot {
        ...
        let ageMs = Date().timeIntervalSince1970 * 1000 - last
